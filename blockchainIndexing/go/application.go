...
func main() {
	...
	versionOld := flag.Int("vOld", 1, "version to query for point query")
	startOld := flag.Int("startOld", 1, "start version for version query")
	endOld := flag.Int("endOld", 1, "end version for version query")
	flag.Parse()
	...
	case "pointQueryOld":
		pointQueryOld(contract, *key, *versionOld)
	case "versionQueryOld":
		versionQueryOld(contract, *key, *startOld, *endOld)
